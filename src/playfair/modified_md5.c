...
void modified_md5(unsigned char *originalblockIn, unsigned char *keyIn,
  ...
  for (i = 0; i < 64; i++) {
    ...
    input = blockIn[4 * j] << 24 | blockIn[4 * j + 1] << 16 |
            blockIn[4 * j + 2] << 8 | blockIn[4 * j + 3];
    printf("Key = %08x\n", A);
    ...
    if (i == 31) {
      ...
      swap(&block_words[(A & (15 << 4)) >> 4],
           &block_words[(B & (15 << 4)) >> 4]);
      swap(&block_words[(A & (15 << 8)) >> 8],
           &block_words[(B & (15 << 8)) >> 8]);
      swap(&block_words[(A & (15 << 12)) >> 12],
           &block_words[(B & (15 << 12)) >> 12]);
    }
  }
